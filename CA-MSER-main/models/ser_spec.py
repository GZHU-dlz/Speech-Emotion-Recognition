...
class BiLSTM_SA(nn.Module):
    ...
    def __init__(self,input_dimen):
        super().__init__()
        self.LSTM = nn.LSTM(input_size=input_dimen,hidden_size=256,num_layers=2,bidirectional=True)
        # 因为是双向 LSTM, 所以要乘2
        self.ffn = nn.Linear(512,128)
        self.relu = nn.ReLU()
        self.classifier = nn.Linear(128,64)
...
class TemporalMfccAwareBlock(nn.Module):
    def __init__(self, nb_filters, kernel_size,dilation,dropout_rate):
        super(TemporalMfccAwareBlock, self).__init__() #现149原来都是300
        ...
        self.conv1_1 = nn.Conv1d(in_channels=149, out_channels=nb_filters, kernel_size=kernel_size,stride=kernel_size-1, padding=0,dilation=dilation)
        ...
        self.conv_original_x = nn.Conv1d(in_channels=149, out_channels=nb_filters, kernel_size=1,padding=0)
