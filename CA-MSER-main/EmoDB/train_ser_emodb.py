...
import argparse
import pickle
from emodb_data_utils import SERDataset
import torch
import numpy as np
# from model import SER_AlexNet, SER_AlexNet_GAP, SER_CNN
# 模型还没有改,先用原来的(维度那些要换)
from emodb_model import Ser_Model
...
def parse_arguments(argv):
    ...
    parser.add_argument('--val_id', type=str, default='03',
                        help='ID of speaker to be used as validation')
    parser.add_argument('--test_id', type=str, default='08',
...
def test(mode, params, model, criterion_ce,L1_loss ,criterion_mml, test_dataset, batch_size, device,
    ...
    with tqdm(test_loader) as td:
        for test_batch in td:
            ...
            one_hot_labels = nn.functional.one_hot(test_labels_batch, num_classes=7).float()
            # test loss
            ...
            test_loss = test_loss_ce + test_loss_mml+L1_loss_all
    ...
    test_cor = test_dataset.confusion_matrix_emodb(test_preds)

    results = (test_loss, test_wa * 100, test_ua * 100)

    if return_matrix:
        test_conf = test_dataset.confusion_matrix_emodb(test_preds)
    ...
    for epoch in range(params['num_epochs']):
        ...
        with tqdm(train_loader) as td:
            for train_batch in td:
                ...
                one_hot_labels = nn.functional.one_hot(train_labels_batch, num_classes=7).float()
                ...
                train_loss = train_loss_ce + train_loss_mml+L1_loss_all
