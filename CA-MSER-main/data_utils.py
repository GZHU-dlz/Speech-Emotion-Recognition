...
# 自动格式化方法: Ctrl+Alt+L(解决取消缩进不匹配任何外部缩进级别的问题!!!)
...
class SERDataset:
    ...
    oversample : bool
        Set 'True' to apply random dataset oversampling to balance the classes(设置“True”以应用随机数据集过采样来平衡类)
    ...
    def __init__(self, features_data, num_classes=4,
        ...
        for speaker_id in features_data.keys():
            if speaker_id in [val_speaker_id, test_speaker_id]:
                # 这部分的意思是除了验证集和测试集，其他的都作为训练集
                continue
                # Concatenate spectrograms from speakers in training set(将训练集中说话人频谱进行拼接)
        ...
        # 获取验证频谱(对应val_speaker_id,与上面的不同)
        ...
        # 将数据集中的数值调整到 [0, 1] 的范围，以便它们可以作为图像像素值使用。
        self._normalize('minmax')

        # 对训练数据集进行随机过采样(通过增加少数类样本的数量来改善不平衡数据集的训练效果。适当地使用随机过采样可以提高模型的性能和泛化能力)
        # Random oversampling on training dataset
        if oversample == True:
            print('\nPerform training dataset oversampling')
            # 有标签训练，所以有train_lebels参数设置
        ...
        # 下面assert目的:这条断言语句的目的是确保 self.train_spec_data 中的样本数量与预期的样本数量相同。
        # 这样可以在训练模型之前，确认数据的完整性和一致性，避免因数据不匹配而导致的错误。
    ...
    def _spec_to_gray(self, data):
        ...
        Convert normalized spectrogram to 3-channel gray image (identical data on each channel)
            and apply AlexNet image pre-processing
            将归一化的声谱图（spectrogram）转换为三通道（RGB）灰度图像，并对其进行与AlexNet模型相兼容的图像预处理。
