...
# model111 = AutoModel(model="iic/emotion2vec_plus_base", hub="ms", disable_update=True)
# ckpt_path = "./emotion2vec_base.pt"
# model111.model.load_state_dict(torch.load(ckpt_path, map_location=device), strict=False)
def main(args):
...
def test(mode, params, model, criterion_ce,L1_loss,criterion_mml, test_dataset, batch_size, device,
         return_matrix=False):
    """Test an SER model.
    ...
    with tqdm(test_loader) as td:
        for test_batch in td:
            ...
            one_hot_labels = nn.functional.one_hot(test_labels_batch, num_classes=4).float()

            # test loss
            test_loss_ce = criterion_ce(test_outputs['M'], test_labels_batch)
            test_loss_mml = criterion_mml(test_outputs['M'], test_labels_batch)
            L1_loss_all=L1_loss(test_outputs['M'], one_hot_labels)
            # test_loss_icd=criterion_ce(test_outputs['F1'], test_labels_batch)
            # test_loss_all=criterion_ce(test_outputs['F2'], test_labels_batch)
            # L1_loss_all=L1_loss(test_outputs['F2'], one_hot_labels)
            # L1_loss_icd=L1_loss(test_outputs['F1'], one_hot_labels)
            test_loss = test_loss_ce + test_loss_mml+0.2*L1_loss_all
...
def train(dataset, params, save_label='default'):
    ...
    scheduler=torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=25,T_mult=1,eta_min=0.00001)
    ...
    for epoch in range(params['num_epochs']):
        ...
        with tqdm(train_loader) as td:
            for train_batch in td:
                ...
                label_model.append(train_labels_batch)
                # Forward pass
                ...
                one_hot_labels = nn.functional.one_hot(train_labels_batch, num_classes=4).float()

                train_preds.append(f.log_softmax(outputs['M'], dim=1).cpu().detach().numpy())
                # print(train_labels_batch.shape) 大小为[64]
                # Compute the loss, gradients, and update the parameters
                train_loss_ce = criterion_ce(outputs['M'], train_labels_batch)
                L1_loss_all=L1_loss(outputs['M'], one_hot_labels)
                # train_loss_icd=criterion_ce(outputs['F1'], train_labels_batch)
                # train_loss_wavlm=criterion_ce(outputs['F2'], train_labels_batch)
                # L1_ICD=L1_loss(outputs['F1'], one_hot_labels)
                # L1_all=L1_loss(outputs['F2'], one_hot_labels)
                train_loss_mml = criterion_mml(outputs['M'], train_labels_batch)
                train_loss = train_loss_ce + train_loss_mml+0.2*L1_loss_all
        ...
        with torch.no_grad():
            val_result = test('VAL', params,
                              model, criterion_ce,L1_loss,criterion_mml, val_dataset,
    ...
    with torch.no_grad():
        ...
        test_result, confusion_matrix = test('TEST', params,
                                             model, criterion_ce,L1_loss,criterion_mml, test_dataset,
